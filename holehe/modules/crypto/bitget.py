...
async def bitget(email, client, out):
    name = "bitget"
    domain = "bitget.com"
    method = "register"
    ...
    headers = {
        ...
        'Referer': 'https://www.bitget.com/register',
    }

    try:
        data = {"email": email}
        response = await client.post(
            'https://api.bitget.com/api/user/v1/register/check',
            headers=headers,
            json=data,
            timeout=10
        )
        ...
        response2 = await client.post(
            'https://api.bitget.com/api/user/v1/forgot/send-email',
            headers=headers,
            json=data2,
            timeout=10
        )
