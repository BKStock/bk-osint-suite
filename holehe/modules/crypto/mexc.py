...
async def mexc(email, client, out):
    name = "mexc"
    domain = "mexc.com"
    method = "register"
    ...
    headers = {
        ...
        'Referer': 'https://www.mexc.com/register',
    }

    try:
        data = {"email": email, "accountType": "email"}
        response = await client.post(
            'https://www.mexc.com/api/platform/account/checkAccount',
            headers=headers,
            json=data,
            timeout=10
        )
        ...
        response2 = await client.post(
            'https://www.mexc.com/api/platform/account/forgotPassword/sendEmail',
            headers=headers,
            json=data2,
            timeout=10
        )
