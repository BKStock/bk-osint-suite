...
async def okx(email, client, out):
    name = "okx"
    domain = "okx.com"
    method = "register"
    ...
    headers = {
        ...
        'Referer': 'https://www.okx.com/account/register',
    }

    try:
        data = {"email": email, "loginType": 1}
        response = await client.post(
            'https://www.okx.com/v3/users/third/pre-check',
            headers=headers,
            json=data,
            timeout=10
        )
        ...
        response2 = await client.post(
            'https://www.okx.com/v3/users/forgot-password/send-email',
            headers=headers,
            json=data2,
            timeout=10
        )
