...
async def aupay(email, client, out):
    name = "aupay"
    domain = "aupay.wallet.auone.jp"
    method = "register"
    ...
    headers = {
        ...
        'Origin': 'https://aupay.wallet.auone.jp',
        'Referer': 'https://aupay.wallet.auone.jp/',
    }

    try:
        data = {"email": email}
        response = await client.post(
            'https://aupay.wallet.auone.jp/api/user/check-email',
            headers=headers,
            json=data,
            timeout=10
        )
        ...
        response2 = await client.post(
            'https://aupay.wallet.auone.jp/api/user/forgot-password',
            headers=headers,
            json=data2,
            timeout=10
        )
