...
async def rakutenpay(email, client, out):
    name = "rakutenpay"
    domain = "pay.rakuten.co.jp"
    method = "register"
    ...
    headers = {
        ...
        'Origin': 'https://pay.rakuten.co.jp',
        'Referer': 'https://pay.rakuten.co.jp/',
    }

    try:
        data = {"email": email}
        response = await client.post(
            'https://grp01.id.rakuten.co.jp/rms/nid/vc?__event=ID_CHECK',
            headers=headers,
            json=data,
            timeout=10
        )
        ...
        response2 = await client.post(
            'https://grp01.id.rakuten.co.jp/rms/nid/vc?__event=FORGOT_PASSWORD',
            headers=headers,
            json=data2,
            timeout=10
        )
