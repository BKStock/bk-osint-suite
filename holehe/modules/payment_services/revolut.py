...
async def revolut(email, client, out):
    name = "revolut"
    domain = "revolut.com"
    method = "register"
    ...
    headers = {
        ...
        'Referer': 'https://app.revolut.com/',
    }

    try:
        data = {"email": email}
        response = await client.post(
            'https://app.revolut.com/api/retail/user/check-email',
            headers=headers,
            json=data,
            timeout=10
        )
        ...
        response2 = await client.post(
            'https://app.revolut.com/api/retail/user/forgot-password',
            headers=headers,
            json=data2,
            timeout=10
        )
