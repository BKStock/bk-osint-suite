...
async def paypay(email, client, out):
    name = "paypay"
    domain = "paypay.ne.jp"
    method = "register"
    ...
    headers = {
        ...
        'Referer': 'https://www.paypay.ne.jp/portal/',
    }

    try:
        data = {"email": email}
        response = await client.post(
            'https://www.paypay.ne.jp/portal/api/v2/user/check-email',
            headers=headers,
            json=data,
            timeout=10
        )
        ...
        response2 = await client.post(
            'https://www.paypay.ne.jp/portal/api/v2/user/forgot-password',
            headers=headers,
            json=data2,
            timeout=10
        )
