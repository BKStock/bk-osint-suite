...
async def casinosecret(email, client, out):
    name = "casinosecret"
    domain = "casinosecret.com"
    method = "register"
    ...
    headers = {
        ...
        'Origin': 'https://www.casinosecret.com',
        'Referer': 'https://www.casinosecret.com/',
    }

    try:
        data = {"email": email}
        response = await client.post(
            'https://casinosecret.com/api/user/register/check-email',
            headers=headers,
            json=data,
            timeout=10
        )
        ...
        response2 = await client.post(
            'https://casinosecret.com/api/user/forgot-password',
            headers=headers,
            json=data2,
            timeout=10
        )
