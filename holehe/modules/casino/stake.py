...
async def stake(email, client, out):
    name = "stake"
    domain = "stake.com"
    method = "register"
    ...
    headers = {
        ...
        'Origin': 'https://www.stake.com',
        'Referer': 'https://www.stake.com/',
    }

    try:
        data = {"email": email}
        response = await client.post(
            'https://stake.com/api/user/register/check-email',
            headers=headers,
            json=data,
            timeout=10
        )
        ...
        response2 = await client.post(
            'https://stake.com/api/user/forgot-password',
            headers=headers,
            json=data2,
            timeout=10
        )
