...
async def bcgame(email, client, out):
    name = "bcgame"
    domain = "bc.game"
    method = "register"
    ...
    headers = {
        ...
        'Origin': 'https://www.bc.game',
        'Referer': 'https://www.bc.game/',
    }

    try:
        data = {"email": email}
        response = await client.post(
            'https://bc.game/api/user/register/check-email',
            headers=headers,
            json=data,
            timeout=10
        )
        ...
        response2 = await client.post(
            'https://bc.game/api/user/forgot-password',
            headers=headers,
            json=data2,
            timeout=10
        )
